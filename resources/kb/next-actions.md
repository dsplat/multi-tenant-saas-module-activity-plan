---
module: activity_plan
audience: internal
locale: zh
title: 计划编排模块典型后续动作
---

# 计划编排（ActivityPlan）典型后续动作

## 策划草稿后（`activity_plan_draft`）

- 与用户逐项确认计划内容（阶段、任务、时间锚点），迭代修改草稿
- 确认无误后 `activity_plan_commit` 定稿：编译为定时任务并自动进入持续跟进（tracked）

## 计划定稿后（`activity_plan_commit`）

- `activity_status` 随时查看任务执行进度与待确认项
