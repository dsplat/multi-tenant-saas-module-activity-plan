 ...
 *
 * Playbook 为活动方法论模板，提供 phases/tasks 骨架供 activity_plan_draft 工具参考。
 * 模式完全照搬 TaskChainRegistry：
 * - 框架内置 demo playbook + 下游 extra_playbook_classes 扩展
 * - key 冲突时下游覆盖框架内置
 * - 定义结构校验在首次访问时完成，坏定义记日志跳过
 *
 * 扩展点：config('ai.activity_plan.extra_playbook_classes') → 静态 playbooks(): array
 */
class PlaybookRegistry
{
    ...
    private function rawDefinitions(): array
    {
        ...
        foreach ((array) config('ai.activity_plan.extra_playbook_classes', []) as $class) {
