...
use MultiTenantSaas\Modules\ActivityPlan\Notifications\ThreadHealthAlertNotification;
...
class ThreadHealthCheckCommand extends Command
{
    ...
    {
        if (! config('ai.activity_plan.enabled', false) || ! config('ai.brain.enabled', false)) {
            $this->warn('项目大脑未启用（ai.activity_plan.enabled / ai.brain.enabled），退出。');
    ...
    private function loadTrackedPlans()
    {
        return TenantScope::allowUnscoped(function () {
            return ActivityPlan::withoutGlobalScope(TenantScope::class)
                ->where('metadata->tracked', true)
                ->whereIn('status', [
                    ActivityPlan::STATUS_PLANNING,
                    ActivityPlan::STATUS_SCHEDULED,
                    ActivityPlan::STATUS_RUNNING,
                ])
                ->get();
        });
    }
     ...
     */
    private function inspect(ActivityPlan $plan, Carbon $now): array
    {
        $unfinished = [ActivityTask::STATUS_PENDING, ActivityTask::STATUS_AWAITING_CONFIRM];

        $tasks = ActivityTask::where('plan_id', $plan->plan_id)->get();
        ...
        $failed = $tasks->where('status', ActivityTask::STATUS_FAILED)->count();
     ...
     */
    private function persistHealth(ActivityPlan $plan, array $health): void
    {
     ...
     */
    private function notify(ActivityPlan $plan, array $health): void
    {
