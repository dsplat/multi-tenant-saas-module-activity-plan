...
namespace MultiTenantSaas\Modules\ActivityPlan\Http\Controllers;
...
/**
 * 活动排期管理（租户管理端，权限 setting.update）
 *
 * 计划 CRUD + 编译 + 取消；任务批准/驳回/完成。
 * 范式对齐 IbotAdminController。
 */
class ActivityPlanAdminController extends Controller
{
    ...
    {
        $plans = ActivityPlan::withCount('tasks')
    ...
    {
        $plan = ActivityPlan::where('plan_id', $planId)->first();
    ...
    /**
     * 编译计划（plan_doc → activity_tasks，幂等）
     */
    ...
    public function cancel(int $planId): JsonResponse
    {
        ...
        if (in_array($plan->status, [ActivityPlan::STATUS_CLOSED, ActivityPlan::STATUS_CANCELLED], true)) {
            return response()->json(['success' => false, 'message' => '计划已终结'], 422);
        }

        // 非终态任务全部取消
        ActivityTask::where('plan_id', $plan->plan_id)
            ->whereNotIn('status', [
                ActivityTask::STATUS_DONE,
                ActivityTask::STATUS_FAILED,
                ActivityTask::STATUS_SKIPPED,
                ActivityTask::STATUS_CANCELLED,
            ])
            ->update(['status' => ActivityTask::STATUS_CANCELLED]);

        $plan->update(['status' => ActivityPlan::STATUS_CANCELLED]);
    ...
    {
        $task = ActivityTask::where('task_id', $taskId)->first();

        if (! $task) {
            return response()->json(['success' => false, 'message' => '任务不存在'], 404);
        }

        if ($task->status !== ActivityTask::STATUS_AWAITING_CONFIRM) {
    ...
    public function rejectTask(int $taskId): JsonResponse
    {
        ...
        $task->update(['status' => ActivityTask::STATUS_SKIPPED]);
    ...
    public function completeTask(Request $request, int $taskId): JsonResponse
    {
        ...
        if ($task->status !== ActivityTask::STATUS_RUNNING) {
    ...
    public function tasksIndex(Request $request): JsonResponse
    {
        ...
        $tasks = $query->get()->map(fn (ActivityTask $t) => [
            'task_id' => $t->task_id,
            'plan_id' => $t->plan_id,
            ...
            'status' => $t->status,
            'remind' => $t->execution_mode === ActivityTask::MODE_REQUIRE_CONFIRM,
        ]);
    ...
    public function storeManualPlan(Request $request): JsonResponse
    {
        ...
        $plan = ActivityPlan::create([
            ...
            'status' => ActivityPlan::STATUS_SCHEDULED,
            'created_by' => (int) $operator->operator_id,
        ]);
    ...
    public function addTask(Request $request, int $planId): JsonResponse
    {
        ...
        $task = ActivityTask::create([
            ...
            'action' => ['type' => 'human'],
            'execution_mode' => $remind ? ActivityTask::MODE_REQUIRE_CONFIRM : ActivityTask::MODE_AUTO,
            'depends_on' => [],
    ...
    public function updateTask(Request $request, int $taskId): JsonResponse
    {
        ...
        if (array_key_exists('remind', $validated) && $validated['remind'] !== null) {
            $attributes['execution_mode'] = $validated['remind']
                ? ActivityTask::MODE_REQUIRE_CONFIRM
                : ActivityTask::MODE_AUTO;
        }

        // 一键完成（todo 语义：从 pending/awaiting_confirm/running 直接置 done）
        if (($validated['status'] ?? null) === 'done') {
            $attributes['status'] = ActivityTask::STATUS_DONE;
    ...
    public function deletePlan(int $planId): JsonResponse
    {
        ...
        ActivityTask::where('plan_id', $plan->plan_id)->delete();
        $plan->delete();
